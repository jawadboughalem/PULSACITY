...
import { colourFamilies, colourNames, readTokens, resolve } from './tokens';
...
/**
 * The two surfaces that cannot read `tokens.css`: the OG image is rendered by a
 * runtime with no stylesheet, and a favicon is a standalone file. They are the
 * only places allowed a colour literal — and every literal in them is checked
 * against the palette below.
 */
const MIRRORS = ['apps/corporate/src/app/opengraph-image.tsx', 'apps/corporate/src/app/icon.svg'];
